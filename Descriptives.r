# Adapted from https://community.mis.temple.edu/mis2502sec004s2019/
# This is just a demonstration of what you can do using R, and to get you used to the syntax.
# In most cases, you'll just modify parameters in these scripts, but sometimes you'll need to change the statements themselves to get it to work with your own datasets.
...
# Import your dataset
dataSet <- read.csv("startup_funding-project.csv")
...
describeBy(dataSet$AmountUSD,dataSet$CityLocation)
...
subset <- dataSet[ which(dataSet$CityLocation=='Bangalore' |  dataSet$CityLocation=='Mumbai'), ]
...
cat("\n###### Performing a t-test: ######\n")
t.test(subset$AmountUSD~subset$CityLocation)

# 7)Using describeBy() to compare the flight distance (Distance) across airlines (using UniqueCarrier).
# describeBy(dataSet$Distance,dataSet$UniqueCarrier)

# 8)Using describeBy() to compare the taxiing out time (TaxiOut) across origin airports (Origin).
# describeBy(dataSet$TaxiOut,dataSet$Origin)

#Do planes spend more time taxiing out to the runway in Newark (EWR) or Philadelphia(PHL) as the origin airport? (using TaxiOut as the taxiing out time, and Origin as the origin airport); 
subset <- dataSet[ which(dataSet$CityLocation=='Bangalore' |  dataSet$CityLocation=='Mumbai'), ]
t.test(subset$AmountUSD~subset$CityLocation)
...
# FOR YOUR OWN DATASET, ADJUST YOUR VARIABLES ACCORDINGLY!!
hist(dataSet$AmountUSD, breaks=4, col="lightgrey", xlab="Amounts in USD", main="Histogram of Amounts in USD")
...
# FOR YOUR OWN DATASET, ADJUST YOUR VARIABLES ACCORDINGLY!!
pdf("histogram.pdf")
hist(dataSet$AmountUSD, breaks=4, col="lightgrey", xlab="Frequency of Amounts in 
     USD of funding received")
dev.off()
