...
MyTree <- rpart(FiveMillionOrMore ~ Industry +InMegaCity+ BangaloreBased+ TechRelated+ PrivateOrSeed +Year+ TopInvestor, data=trainingSet, method="class", 
...
table(Predicted=predTraining,Observed=trainingSet[, 21] )
...
table(Predicted=predValidation,Observed=validationSet[, 21] )
...
predRateTraining <- mean(predTraining == trainingSet[, 21])
predRateValidation <- mean(predValidation == validationSet[, 21])
